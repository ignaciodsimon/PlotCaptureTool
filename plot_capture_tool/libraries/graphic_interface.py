...
def startGraphicInterface():
    ...
    Button(rootWindowHandler, text="Capture curve points from image", command=captureCurvesCallback).pack()
...
#    Button(rootWindowHandler, text="Computations from curves", command=computationsFromCurves).pack()
    ...
    rootWindowHandler.title("Plot capture tool - I. D. Simon, 2016.")
...
def displayCurvesCapturing(filename):
    ...
    d = InputDialog(rootWindowHandler, "Min. value on X-axis:")
    rootWindowHandler.wait_window(d.top)
    min_voltage = outputDataVar
    d = InputDialog(rootWindowHandler, "Max. value on X-axis:")
    rootWindowHandler.wait_window(d.top)
    max_voltage = outputDataVar
    
    d = InputDialog(rootWindowHandler, "Min. value on Y-axis:")
    rootWindowHandler.wait_window(d.top)
    min_current = outputDataVar
    d = InputDialog(rootWindowHandler, "Max. value on Y-axis:")
    rootWindowHandler.wait_window(d.top)
    max_current = outputDataVar

    plot.title("Step 1 - Select the origin of coordinates of the plot (point 0,0) ...")
...
def onCapturePlotWindowClosedEvent(event):
    ...
    max_x_axis = curvesCorners[1][0]
    min_x_axis = curvesCorners[0][0]
    max_y_axis = curvesCorners[2][1]
    min_y_axis = curvesCorners[0][1]
    ...
    print " >> Captured data <<\n"
    for index in range(len(curvesVoltages)):
        print "\n> Curve value:", curvesVoltages[index]
        print "> Curve data:", curvesData[index]
...
def onCapturePlotClickedEvent(event):
    ...
    if len(curvesCorners) < 3:
        curvesCorners.extend([[event.xdata, event.ydata]])
        if len(curvesCorners) == 1:
            updatedPlotText = "Step 1 - Select the maximum X-axis point ..."
        if len(curvesCorners) == 2:
            updatedPlotText = "Step 1 - Select the maximum Y-axis point ..."
                ...
                if event.dblclick == 1:
                    if not len(curvesData) == 0:
                        if not len(curvesData[-1]) == 0:
                            ...
                            d = InputDialog(rootWindowHandler, "Value of inserted curve:")
                            rootWindowHandler.wait_window(d.top)
...
#        yCoords.extend([yCoords[0]])
        plot.plot([curvesCorners[0][0], curvesCorners[1][0]], [curvesCorners[0][1], curvesCorners[1][1]], 'blue')
        plot.plot([curvesCorners[0][0], curvesCorners[2][0]], [curvesCorners[0][1], curvesCorners[2][1]], 'blue')
